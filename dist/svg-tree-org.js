(function (global, factory) {
  ...
  (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.SvgTreeOrg = {}, global.compositionApi));
  ...
  var script$1 = compositionApi.defineComponent({
      ...
      setup(props, { emit }) {
          const { node, fontSize, letterSpacing } = compositionApi.toRefs(props);
          const middle = compositionApi.computed(() => node.value.xStart + node.value.width / 2); // 中间位置
          const verticalMiddle = compositionApi.computed(() => node.value.yStart + node.value.height / 2);
          const childNodes = compositionApi.computed(() => props.node.children);
          // 设置节点文本
          const nodeText = compositionApi.ref([]);
          const setNodeText = () => {
          ...
          // 节点文字
          const startY = compositionApi.computed(() => node.value.yStart + paddingSize + fontSize.value);
          const startX = compositionApi.computed(() => node.value.xStart + fontSize.value);
          const setAttrs = (i) => {
          ...
          const line2Dth = compositionApi.ref(''); // 折叠节点上或者下方的竖线：line2      竖线
          const line1Dth = compositionApi.ref(''); // 折叠节点下方的横线：向左画（第一个节点不需要画）    横线
          const linesChildDth = compositionApi.ref(''); // 与子节点的第一段连线
          const collaspeStartY = compositionApi.ref(0);
          const collaspeVerticalStartY = compositionApi.ref(0);
          const createLine = () => {
          ...
          const linkLineDth = compositionApi.ref(''); // 跨节点连接线
          const linkLineColor = compositionApi.ref('');
          const linkNodeData = compositionApi.inject('linkNodeData');
          const treeData = compositionApi.inject('treeData');
          const rootNodesep = compositionApi.inject('rootNodesep');
          ...
          compositionApi.watch(() => props.treeDirection, (val) => {
              setNodeText();
              createLine();
          });
  ...
  var script = compositionApi.defineComponent({
      ...
      setup(props, { emit }) {
          const svgRef = compositionApi.ref(null);
          const treeBox = compositionApi.ref(null);
          ...
          const treeWidth = compositionApi.ref(0);
          const treeHeight = compositionApi.ref(0);
          const viewBox = compositionApi.ref('0 0 0 0');
          let svgWidth = 0;
          let svgHeight = 0;
          // 设置节点坐标和svg宽高
          const setAxis = () => {
          ...
          // 跨节点连接
          const setNodeLink = () => {
              compositionApi.provide('linkNodeData', props.linkNodeData);
              compositionApi.provide('treeData', currentTreeData);
              compositionApi.provide('rootNodesep', props.rootNodesep);
          };
          ...
          compositionApi.watch(() => props.direction, (val) => {
              setAxis();
          });
          compositionApi.onMounted(() => {
  ...
  exports.CustomTreeOrg = CustomTreeOrg;
